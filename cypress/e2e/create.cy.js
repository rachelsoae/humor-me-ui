  ...
  it.only('Should submit a poster based on user input', () => {
    ...
    cy.wait('@getQuotes').wait('@getImages').wait('@getPosters').then((interception) => {
      cy.get('.form-button').first().click()
        ...
        .get('.poster-buttons').children().last()
    })
    // cy.wait('@getQuotes').wait('@getImages').wait('@getPosters').then((interception) => {
    //   cy.get('.nav-buttons').children().eq(1).click()
    //     .url().should('eq', 'http://localhost:3000/favorites')
    //     .get('.cards-grid').children().should('have.length', 4)
    //     .get('.cards-grid').children().last().contains('.emojis', '😈')
    // })
  })
})